...
function getFakeRoutes(providerName, logo) {
    // Generate visual route structure for UI
...
// ==================================================================
// 3. REAL INTEGRATIONS (WITH RICH CONTEXT)
// ==================================================================
...
async function getLifiQuote(params, amount, fromChain, toChain) {
    try {
        ...
        // --- CONSTRUCT RICH CONTEXT (CRITICAL FOR UI) ---
        // We recreate the "lifiQuoteResultCtx" structure OneKey uses
        const richCtx = {
            lifiQuoteResultCtx: {
                stepInfo: step, 
        ...
        // Extract Fees from LiFi response for UI display
        ...
        return {
            ...
            routesData: [], // LiFi app logic builds routes from `richCtx`
            ctx: richCtx,
            fiatFee: parseFloat(fiatFee) + parseFloat(gasCostUSD)
        };
    } catch (e) { return null; }
}
...
async function generateAllQuotes(params, eventId) {
    ...
    } catch { amount = ethers.parseUnits(Number(amount).toFixed(18), 18).toString(); }
...
function formatQuote(providerConf, params, data, eventId, isBest) {
    ...
    return {
        ...
        fee: { 
            ...
            estimatedFeeFiatValue: data.fiatFee || 0.1,
            protocolFees: 0 // Adding this helps UI
        },
        routesData: data.routesData,
        // PASS FULL CONTEXT
        quoteResultCtx: { tx: data.tx, providerId: providerConf.id, isMock: false, ...data.ctx },
        allowanceResult: null,
        // ENABLE RECEIVING ADDRESS CHANGE
...
function getMockQuote(providerConf, params, eventId, isBest) {
    ...
    return {
        ...
        gasLimit: 21000, quoteId: uuidv4(), eventId, isBest
    };
}
...
app.post('/swap/v1/build-tx', jsonParser, (req, res) => {
    ...
    try {
        ...
        return res.json(ok({
            ...
            ctx: quoteResultCtx, // PASS FULL RICH CONTEXT BACK
...
app.listen(PORT, () => console.log(`Bitrabo v89 (Rich Data) Running on ${PORT}`));
