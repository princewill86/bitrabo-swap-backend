...
// ==================================================================
// 1. HELPERS (Updated with Strict Hex)
// ==================================================================

// ⚡ v102: Robust Hex Converter (Fixes the "Spinning" issue)
function toHexStrict(val) {
    ...
    try {
        ...
        // Otherwise convert from decimal/BigNumber
...
app.post('/swap/v1/build-tx', jsonParser, (req, res) => {
    ...
    try {
        // ⚡ HEX FORCE: Frontend will spin if it gets a Decimal string.
        const gasHex = toHexStrict(quoteResultCtx.tx.gas || quoteResultCtx.tx.gasLimit || 500000);
        const valHex = toHexStrict(quoteResultCtx.tx.value);
        const gasPriceHex = quoteResultCtx.tx.gasPrice ? toHexStrict(quoteResultCtx.tx.gasPrice) : undefined;

        // ⚡ DUAL GAS: Send both 'gas' and 'gasLimit' to be safe
        const finalTx = {
            ...quoteResultCtx.tx,
            from: userAddress,
            value: valHex,
            gas: gasHex,      // Some wallets want this
            gasLimit: gasHex, // Ethers wants this
            gasPrice: gasPriceHex
        };

        const feeAmount = new BigNumber(quoteResultCtx.toAmount || 0).multipliedBy(FEE_PERCENT).toFixed(6);

        console.log("   ✅ /build-tx Success. Sending Safe Hex TX.");
        return res.json(ok({
            result: { 
                info: { provider: quoteResultCtx.providerId }, 
                protocol: 'Swap', 
                fee: { percentageFee: FEE_PERCENT * 100 }, 
                gasLimit: Number(new BigNumber(gasHex).toString()), // Return Decimal here for UI display
                routesData: quoteResultCtx.routesData || [],
                oneKeyFeeExtraInfo: { oneKeyFeeAmount: feeAmount, oneKeyFeeSymbol: "TOKEN", oneKeyFeeUsd: "0.10" }
            },
            ctx: quoteResultCtx, // MUST echo this back
            tx: finalTx
        }));
    } catch (e) { 
...
app.listen(PORT, async () => {
    console.log(`Bitrabo v102 (Hex Fix) Running on ${PORT}`);
