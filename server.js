...
// ==================================================================
// 3. CORE HANDLERS (v100 Logic)
// ==================================================================

// ⚡ QUOTE ENDPOINT
// We keep v100 logic exactly, but we modify formatQuote later to save data
...
app.post('/swap/v1/build-tx', jsonParser, (req, res) => {
    ...
    try {
        ...
        // ⚡ RECOVERY: Unpack the data we saved in formatQuote
        // This is what stops the page from spinning.
        ...
        return res.json(ok({
            result: { 
                ...
                oneKeyFeeExtraInfo: { 
                    ...
                    oneKeyFeeSymbol: fromTokenInfo?.symbol || "TOKEN", 
                    oneKeyFeeUsd: "0.10" 
                },
                // ⚡ KEY FIX: Return the fields required for the confirmation screen
                fromTokenInfo, 
                toTokenInfo,
...
// ==================================================================
// 4. PROVIDER INTEGRATIONS (v100 Unchanged)
// ==================================================================
...
async function generateAllQuotes(params, eventId) {
    ...
    const promises = PROVIDERS_CONFIG.map(async (p, i) => {
        ...
        // ⚡ Update: Pass 'params' fully to formatQuote so it can construct token info
...
function formatQuote(providerConf, params, data, eventId, isBest) {
    ...
    // ⚡ PREPARE DATA FOR CTX (Same logic as v100 but stored)
    const fromTokenInfo = { contractAddress: params.fromTokenAddress || "", networkId: params.fromNetworkId, decimals: 18, symbol: "TOKEN" };
    const toTokenInfo = { contractAddress: params.toTokenAddress, networkId: params.toNetworkId, decimals: data.decimals || 18, symbol: "UNK" };
    ...
    return {
        ...
        quoteResultCtx: { 
            ...
            isMock: false, 
            // Saving data for build-tx to reuse
            fromTokenInfo, 
            toTokenInfo,
...
app.listen(PORT, async () => {
    console.log(`Bitrabo v112 (v100 + Data Echo) Running on ${PORT}`);
